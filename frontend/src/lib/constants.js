export const CATEGORIES_CNCCFP = [
    { code: "A1", label: "Imprimés, documents, tracts" },
    { code: "A2", label: "Frais de distribution" },
    { code: "A3", label: "Affiches et frais d'affichage" },
    { code: "B1", label: "Réunions publiques, location de salles" },
    { code: "B2", label: "Frais de réception, traiteur" },
    { code: "C1", label: "Frais postaux et de télécommunications" },
    { code: "D1", label: "Frais de transport et de déplacement" },
    { code: "E1", label: "Frais de personnel" },
    { code: "F1", label: "Frais de gestion et de fonctionnement" },
    { code: "G1", label: "Prestataires de services (conseil, communication)" },
    { code: "H1", label: "Frais financiers" },
    { code: "I1", label: "Divers" },
];

export const PRISES_EN_CHARGE = [
    { value: "mandataire", label: "Payée par le mandataire" },
    { value: "parti", label: "Payée directement par un parti" },
];

export const ORIGINES_CONCOURS = [
    { value: "candidat", label: "Candidat" },
    { value: "parti", label: "Formation politique" },
    { value: "tiers_pp", label: "Tiers personne physique" },
];

export const TYPES_PIECE = [
    { value: "devis", label: "Devis" },
    { value: "facture", label: "Facture" },
];

export const TYPES_RECETTE = [
    { value: "Don", label: "Don (Personne Physique)" },
    { value: "Apport", label: "Apport Personnel" },
    { value: "Pret", label: "Prêt (Personne Physique ou Parti)" },
];

export const STATUTS_DEPENSE = [
    { value: "Engagé", label: "Engagé (Devis signé)" },
    { value: "Facturé", label: "Facturé (En attente de paiement)" },
    { value: "Payé", label: "Payé" },
];
